...
a|icon:lock[title=Fixed at build time] [[quarkus-kafka-client_quarkus-kafka-jmx-enabled]] [.property-path]##link:#quarkus-kafka-client_quarkus-kafka-jmx-enabled[`+++quarkus.kafka.jmx.enabled+++`]##
ifdef::add-copy-button-to-config-props[]
config_property_copy_button:+++quarkus.kafka.jmx.enabled+++[]
endif::add-copy-button-to-config-props[]
...
When disabled, the `AppInfoParser.registerAppInfo` and `unregisterAppInfo` methods no longer register JMX MBean while preserving internal metrics registration.

When disabled, also sets the default value of the `kafka.metric.reporters` property to empty value.


ifdef::add-copy-button-to-env-var[]
Environment variable: env_var_with_copy_button:+++QUARKUS_KAFKA_JMX_ENABLED+++[]
endif::add-copy-button-to-env-var[]
ifndef::add-copy-button-to-env-var[]
Environment variable: `+++QUARKUS_KAFKA_JMX_ENABLED+++`
endif::add-copy-button-to-env-var[]
--
|boolean
|`+++true+++`
...
a|icon:lock[title=Fixed at build time] [[quarkus-kafka-client_quarkus-kafka-devservices-strimzi-server-configs-server-configs]] [.property-path]##link:#quarkus-kafka-client_quarkus-kafka-devservices-strimzi-server-configs-server-configs[`+++quarkus.kafka.devservices.strimzi.server-configs."server-configs"+++`]##
ifdef::add-copy-button-to-config-props[]
